...
export async function GET(req: NextRequest) {
  ...
  const { date, duration, locationId, days } = parsed.data;
  ...
  for (let i = 0; i < days; i++) {
    ...
    const slots = await generateSlots(ctx.windows, duration, { lat: loc.lat, lng: loc.lng }, ctx.existing, ctx.settings, getCommute);
    out.push({
      date: d,
